  ...
  return (
    <>
      <header className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-20 flex flex-col items-center pl-6 pr-6 py-3 backdrop-blur-sm ${headerShapeClass} border border-[#333] bg-indigo-500/6 w-[calc(100%-2rem)] md:w-[800px] transition-[border-radius] duration-0 ease-in-out`}>
        ...
        <div className="grid grid-cols-3 items-center w-full gap-x-6 sm:gap-x-8">
          <div className="flex items-center">
            {logo}
          </div>

          <nav className={`hidden sm:flex items-center space-x-4 sm:space-x-6 text-sm ${user ? 'mx-auto' : ''}`}>
          ...
          <div className='justify-self-end'>
            {user ?
              ...
              :
              <div className="hidden sm:flex justify-end items-center gap-2 sm:gap-3 max-w-48 w-full">
                <Button className='bg-white text-black hover:bg-slate-100 w-20' onClick={handleLogin}> Login </Button>
                <Button className='bg-white text-black hover:bg-slate-100 w-20' onClick={handleSignup}> Signup </Button>
              </div>
            }
          </div>
        ...
        <div className={`sm:hidden flex flex-col items-center w-full transition-all ease-in-out duration-300 overflow-hidden
          ...
          <nav className="flex flex-col items-center space-y-4 text-base w-full">
            ...
            <Button className='bg-white text-black hover:bg-slate-100 max-w-20 w-full' onClick={handleLogin}> Login </Button>
            <Button className='bg-white text-black hover:bg-slate-100 max-w-20 w-full' onClick={handleSignup}> Signup </Button>
          </nav>
        </div>
      </header>
      {/* Auth Form */}
